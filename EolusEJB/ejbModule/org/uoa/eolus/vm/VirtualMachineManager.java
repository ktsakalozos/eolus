...
//		clearAllVMEntries();
	...
	public boolean userOwnsVM(String user, String VMname){
		List<VirtualMachine> ts = em.createQuery("select vm from VirtualMachine vm where vm.name='" + VMname + "'" +
				" and vm.user.id='"+user+"'").getResultList();
		if (ts.size()!=0)
			return true;
		else
			return false;
	...
	public synchronized void Kill(String username, String VMname, Boolean force)
	throws UnknownVMException, InternalErrorException, UnknownUserException {
		if (!userOwnsVM(username, VMname)) 
	...
	public synchronized void Kill(String VMname, Boolean force) throws UnknownVMException, InternalErrorException {
		...
		if (force)
			cloud.vmAction(CloudConstants.VMAction.poweroff, VMID);				
		else
			cloud.vmAction(CloudConstants.VMAction.shutdown, VMID);
	...
	public void migrate(String VMname, int hostID) throws UnknownVMException, InternalErrorException{
		...
		if (hostsource == null)
			throw new InternalErrorException("Host "+hostname+" does not exist!");
		...
		if (hostsource.getSitename().equals(hosttarget.getSitename())){
			...
			if (s == null)
				throw new InternalErrorException("Site "+hostsource.getSitename()+" does not exist!");
			
			livemigration = s.isLiveMigrationCapable();
		}else{
			//TODO:  Cross site migration
