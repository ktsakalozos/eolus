...
public class ClusterPresenter extends Presenter
{
	 ...
	 * 
	 * 
	 *  <SITE><HOSTS><HOST>n30.di.uoa.gr</HOST><HOST>n31.di.uoa.gr</HOST><HOST>n32.di.uoa.gr</HOST>
	 *  <HOST>n33.di.uoa.gr</HOST></HOSTS><PROPERTIES><LIVEMIGRATE>false</LIVEMIGRATE>
	 *  <MIGRATE>true</MIGRATE></PROPERTIES></SITE>
	 */
	...
	public void loadNewCluster()
	{
		...
		/* periodically refresh host names */
		...
		final AsyncCallback<String> callback = new AsyncCallback<String>()
		{
			...
			public void onSuccess(String b) 
			{
				HTML msg = new HTML();
				if(b.equals("ok"))
					msg= HTMLwriter.getSystemMessage("green", "Cluster created successfully." +
					" You can now view it.");
		...
		submit.addClickHandler(new ClickHandler() 
		{
			...
			{ 
				...
				//String itemlist = Parser.createSeparatedListFromListBox(selected,"-");
	...
	public void reload()  //to be used for pagination as well
	{
			...
			public void onSuccess(final String[] currhosts) 
			{
				...
				info.addClickHandler(new ClickHandler()
				{
					public void onClick(ClickEvent event) 
					{
						...
						serverCall.adminGetClusterInfo(name, new AsyncCallback<String[]>() //get hostz
								{
							...
							public void onSuccess(String[] result) //all hosts - current
							{
								if(result!=null)
								{
									LightBox l = new LightBox();
				...
				remove.addClickHandler(new ClickHandler()
				{
					public void onClick(ClickEvent event) 
					{
						...
						serverCall.adminRemoveCluster(name, new AsyncCallback<String>() //get hostz
								{
				...
				add.addClickHandler(new ClickHandler()
				{
					public void onClick(ClickEvent event) 
					{
						...
						serverCall.getHosts(new AsyncCallback<String[]>() //get hostz
								{
			...
			serverCall.adminAddHostToCluster(hosts[i], clustername,new AsyncCallback<String>()
				...
				public void onSuccess(String result)
				{
					...
					if(result.equalsIgnoreCase("ok"))
						...
						msg =HTMLwriter.getSystemMessage("green", "Hosts successfully added");
					else
						msg =HTMLwriter.getSystemMessage("red", errorRequest+" "+result);
	...
	public void loadHost(final String clustername,MyTable table,final String hostname,final int row)
	{
		...
		remove.addClickHandler(new ClickHandler() /* actions on hosts :) */
		{

			public void onClick(ClickEvent event) //remove host from site
			{	serverCall.adminRemoveHostfromCluster(hostname, clustername, new AsyncCallback<String>() //teest
					{
				...
				public void onSuccess(String result)
				{
					...
					if(result.equalsIgnoreCase("ok")) //host removed successfully
					{
