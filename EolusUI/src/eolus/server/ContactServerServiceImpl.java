...
public class ContactServerServiceImpl extends RemoteServiceServlet implements ContactServerService
{
	...
	{ 
		...
		//we have an ArrayList with all the vms
		//String[] vmString = (String[]) vms.toArray(); //java lib magic! :D
	...
	public String getNetworkInfo(String network) throws Exception
	{
		...
		String xml;
		if(isAdmin())
			xml=eolus.adminGetVNetInfo(network);

		else
		{
			//TODO special function for public networks xml = eolus.get
	...
	public String[] getStrayNetworks() throws Exception
	{
		...
		String[] all = eolus.adminGetAllVNetList(); //get all networks 
		String[] users = getUsers();
		String[] nonstray= {};
		String[] stray= {};
		/* get networks that have a user assigned to them */
		for ( String s : users )
		{ 
		...
		/* find out which networks are stray */
	...
	public String[] getNetworksbyUser(String user) throws Exception
	{
		...
		String[] s;
		if(isAdmin())
			s=eolus.adminGetVNetList(user);
		else
			if(user.equals("public"))
				s= eolus.getVNetPublicList();
			else
				s=eolus.getVNetList();
	...
	{
		init();
		return SecurityAssociation.getCallerPrincipal().getName();//session 
	...
	public String[][] getUserTemplates(String user, boolean sync)  throws Exception
	{
		...
		String[] templates;
		if(isAdmin())
			templates=eolus.adminGetTemplates(user);
		else
		{
			if(user.equals("public"))
				templates = eolus.getPublicTemplates();
			else
				templates=eolus.getTemplates();
		...
		result[0][1]="Status";
		result[0][2]="Actions";
		/* and then an array with all the templates */
		int i=1;
		for(String templ:templates)
		{
			...
			if(isAdmin.equals(true))
				status=eolus.adminGetTemplateStatus(user,templ);
			else
			{
				if(user.equals("public"))
					status = eolus.getPublicTemplateStatus(templ);
				else
					status = eolus.getTemplateStatus(templ);
	...
	public String deleteTemplate(String user, String template)  throws Exception
	{
		...
		if(isAdmin())
			eolus.adminRemoveTemplate(user, template);
		else
			eolus.removeTemplate(template);
		return "ok";
	...
	public String moveTemplate(String user, String tempName, String newTempName)  throws Exception
	{
		...
		if(templateNameExists(user,newTempName))
			return "The selected template name already exists";
		if(isAdmin())
			eolus.adminTransferTemplate(user, tempName, newTempName,true);
		else
			eolus.transferTemplate(tempName, newTempName, true);
		return "ok";
	...
	public String copyTemplate(String user, String tempName, String newTempName)  throws Exception
	{
		...
		if(isAdmin())
			eolus.adminTransferTemplate(user, tempName, newTempName, false);
		else
			eolus.transferTemplate(tempName, newTempName, false);
		return "ok";
	...
	public String makeTemplatePublic(String user, String tempName)   throws Exception
	{
		...
		for( String name : eolus.getPublicTemplates() )
			pub_temps.add(name);

		String newName = tempName;

		if( pub_temps.contains(tempName) ) { /*check for duplicates*/
			int i=1;
			while( pub_temps.contains(tempName+"_"+i) )
				i++;
			newName = tempName+"_"+i; /*fancy renaming*/
		}
	...
	public String[] getVMinfo(String vm)   throws Exception
	{
		...
		if(isAdmin.equals(true))
			xml = eolus.adminGetVMInfo(vm);
		else
			xml = eolus.getVMInfo(vm);
	...
	public String[] getUserVmList(String user)  throws Exception
	{
		...
		if(isAdmin())
			vms = eolus.adminGetUserVMlist(user);
		else
			vms = eolus.getVMlist();
		return vms;
	...
	public String actionOnVm(String user, String vm, String action) throws Exception{
		...
		if( "Kill".equalsIgnoreCase(action) )
		{
			...
			if(isAdmin.equals(true))
				eolus.adminShutdownVM(vm, false);
			else
				eolus.shutdownVM(vm, false);
			...
			if(isAdmin.equals(true))
				eolus.adminShutdownVM(vm, true);
			else
				eolus.shutdownVM(vm, true);
...

	{
		...
		if(isAdmin.equals(true))
			eolus.adminCreateVM(user, template, VmName, null, cores, mem, nets);
		else 
			eolus.createVM(template,VmName,cores,mem,nets);
	...
	public String[][] getUserScripts(String user, boolean sync) throws Exception { 
		...
		{
			if(isAdmin())
				eolus.adminSyncUserScripts(user);
			else
				eolus.syncScripts();
		...
		String[] scripts;
		if(isAdmin.equals(true))
			scripts= eolus.adminGetScriptList(user);
		else 
		{
	...
	public String getScriptDescription(String user, String script) throws Exception 
	{
		...
		if(isAdmin())
			return eolus.adminGetDescription(user, script);
		else
			return eolus.getDescription(script);
	...
	public String runScript(String user, String script, String vm)   throws Exception
	{
		...
		if(isAdmin())
			eolus.adminApplyScript(user, script, vm);
		else
			eolus.applyScript(script, vm);
	...
	public String[] runCommand(String user, String vm, String command)   throws Exception
	{
		...
		if(isAdmin())
			return eolus.adminExecCMD(command, vm); //ti epistrefei arages...
		else
			return eolus.execCMD(command, vm);
