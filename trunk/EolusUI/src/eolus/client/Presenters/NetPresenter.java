...
public class NetPresenter extends Presenter {
	...
	public void showinfo(String network)
	{
		...
		AsyncCallback<String> callback = new AsyncCallback<String>()
		{
			...
			public void onSuccess(String xml)
			{
				...
				v.add(new HTML("<strong>Name :</strong>"+name+"<br/><strong>Is Public:</strong>"
						+ispub+"<br/><strong>State :</strong>"+state+
						"<br/><strong>IP Addresses in Use :</strong>"+iplist.length));
				...
				//scr.setAlwaysShowScrollBars(true);
				scr.setHeight("75px");
				disc.setContent(scr);
				v.add(disc);
	...
	public void remove(String network)
	{
		AsyncCallback<String> callback = new AsyncCallback<String>()
		{
			...
			public void onSuccess(String result)
			{
				...
				if(result.equals("ok"))
				{
					/* refresh so that change is immediately visible */
					doRefresh();
				}
				else
				{
		...
		if(user.isAdmin())
			...
			serverCall.removeNetwork(network,callback);

		else
			serverCall.removeNetworkUser(network,callback);
	...
	public void doassign(String network, String usr,final LightBox l)
	{
		...
		AsyncCallback<String> callback = new AsyncCallback<String>() {
			...
			public void onSuccess(String result)
			{
				...
				HTML msg = new HTML();
				if(result.equals("ok"))
					msg = HTMLwriter.getSystemMessage("green", "Network assigned successfully");
				else
					msg = HTMLwriter.getSystemMessage("red", "Problem assigning network: "+result);
				l.add(msg);
				if(result.equals("ok"))
				{
					l.autoclose();
	...
	public void assign(final String network)
	{
		...
		/* add more things to the lightbox */
		VerticalPanel v = new VerticalPanel();
	...
	{
		/* load only the users of the current page*/
	...
	public void loadUser(final String userdata) //load a user's virtual networks
	{
		...
		if(!divExists(userdata)) /*if user table loaded for the first time, create it */
		{
		...
		else /* else update it by overwriting */
		{
			content = (VerticalPanel) this.getContentWidgetNoClear(userdata,divtitle);	
		...
		serverCall.getNetworksbyUser(userdata, new AsyncCallback<String[]>() 
			...
			public void onSuccess(String[] result) 
			{
				...
				if(0==size) /* if table is empty display message */
				{
				...
				else /* add appropriate headers to display user data */
				{
	...
	public void loadVN(final String username,MyTable table,final String network,final int row)
	{
		...
		/* add the action buttons in a single cell */
		...
		actions.add(info);
		if(user.isAdmin())
			actions.add(assign);/* plain users can't assign networks to other users */
	...
	public void LoadCreateNetwork(){
			...
			public void onClick(ClickEvent event) {
				...
				if(Parser.isvalidIP(ip.getText())){
					...
					while(i<lb.getItemCount()){
						...
						if(temp.equals(ip.getText())){ //ip already exists in list
					...
					ip.setText("");
					error_ip.setVisible(false); //now the list is not empty
		...
		submit.addClickHandler(new ClickHandler()
		{
			public void onClick(ClickEvent event)
			{
				submit(map_info); /* we pass all the data to a separate function via the HashMap*/
	...
	{ /* handle network creation */
		...
		AsyncCallback<String> callback = new AsyncCallback<String>()
		{
			...
			public void onSuccess(String b) {
				...
				HTML msg = new HTML();
				if(b.equals("ok"))
					msg= HTMLwriter.getSystemMessage("green", "Network created successfully." +
					" You can now view it.");
				//refresh teh page
				else{  
