...
public class Nest {
	...
	public File matchVM(String user,String VMTemplateClass, String VMname, String hostName, int cores, int memSize, 
		...
		try {
			...
			out.write(""
					...
					+ " target   = \"xvda1\"] \n"
					+ "DISK   = ["
					+ " source   = \"" + repo +"/"+ user +"/" + VMTemplateClass + "/disk.img\", \n" 
					+ " target   = \"xvda2\", \n "
					+ " readonly = \"no\" ] \n");
			...
			if (extraImages!=null){
				for (int i = 0; i < extraImages.length; i++) {
					try{
						...
						out.write("DISK   = ["
								+ " source   = \"" + repo +"/"+ user +"/" + VMTemplateClass + "/"+extraImages[i]+"\", \n" 
								+ " target   = \"xvda"+j+"\", \n "
								+ " readonly = \"no\" ] \n");
					}catch(Exception x){
	...
	public File matchVM(String user, String mclass, String mname, int cores, int memSize,
			String[] nets) throws DirectoryException {
	...
	public void copyUserTemplate(String user, String template, String target, boolean move) throws DirectoryException {
		String cmd = "mv";
		if (move)
			cmd = "mv " + repo + "/" + user + "/" + template + " " + repo + "/" + user + "/" + target + "; exit; \n";
		else
			cmd = "cp -r " + repo +  "/" + user + "/" + template + " " + repo +  "/" + user + "/."
			+ target + "; mv " + repo +  "/" + user + "/." + target + " " + repo
			+  "/" + user + "/" + target + "; exit; \n";
	...
	public void copyUserToUserTemplate(String fromuser, String touser, String template, String target, boolean move) throws DirectoryException {
		String cmd = "mv";
		if (move)
			cmd = "mv " + repo + "/" + fromuser + "/" + template + " " + repo + "/" + touser + "/" + target + "; exit; \n";
		else
			cmd = "cp -r " + repo +  "/" + fromuser + "/" + template + " " + repo +  "/" + touser + "/."
			+ target + "; mv " + repo +  "/" + touser + "/." + target + " " + repo
			+  "/" + touser + "/" + target + "; exit; \n";
	...
	public File scheduleVMtoHost(String vMtemplateOwner, String vMclass,
			String vMname, String sitedesc, String host, Integer cores,
			Integer memSize, String[] nets) throws IOException, DirectoryException {
		File desc = new File(repo + "/" + vMtemplateOwner + "/" +vMclass + "/"
				+sitedesc);
	...
	public File newVMScheduling(String vMtemplateOwner, String vMclass,
			String vMname, Integer cores, Integer memSize,
			String[] nets) throws IOException, DirectoryException, InternalErrorException {
		...
		while (sitesinfo.size()!=0){
			...
			while(hostinfo.size() != 0){
				...
				if (isEnough(hi,cores, memSize)){
					return scheduleVMtoHost(vMtemplateOwner, vMclass, vMname,
						sitename, hi.name, cores, memSize, nets);					
				}else{
